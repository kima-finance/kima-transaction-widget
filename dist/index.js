...
import React171, { useEffect as useEffect25 } from "react";
import { useDispatch as useDispatch28, useSelector as useSelector43 } from "react-redux";
...
import React170, { useEffect as useEffect24, useState as useState21, useRef as useRef10 } from "react";
import { useDispatch as useDispatch27, useSelector as useSelector42 } from "react-redux";
...
// src/components/TransferWidget.tsx
import { toast as toast10, Toaster as Toaster2 } from "react-hot-toast";

// plugins/solana/components/SolanaWalletConnectModal.tsx
import React166 from "react";
import { useDispatch as useDispatch23, useSelector as useSelector38 } from "react-redux";

// plugins/solana/components/SolanaWalletSelect.tsx
import React164, { useEffect as useEffect22, useMemo as useMemo24, useRef as useRef8 } from "react";
import { useDispatch as useDispatch21, useSelector as useSelector36 } from "react-redux";
...
var SolanaWalletSelect = () => {
  ...
  const [detected, undetected] = useMemo24(() => {
    const detected2 = [];
    const undetected2 = [];
...
import React165, { useMemo as useMemo25 } from "react";
import { useDispatch as useDispatch22, useSelector as useSelector37 } from "react-redux";
...
import { useDispatch as useDispatch26, useSelector as useSelector41 } from "react-redux";

// plugins/tron/components/AccountDetailsModal.tsx
import React167, { useMemo as useMemo26 } from "react";
import { useDispatch as useDispatch24, useSelector as useSelector39 } from "react-redux";
...
var AccountDetailsModal2 = () => {
  ...
  const networkDetails = useMemo26(
...
import React168, { useEffect as useEffect23, useMemo as useMemo27, useRef as useRef9 } from "react";
import { useDispatch as useDispatch25, useSelector as useSelector40 } from "react-redux";
...
var TronWalletSelect = () => {
  ...
  const [detected, undetected] = useMemo27(() => {
    const detected2 = [];
    const undetected2 = [];
    for (const wallet of wallets) {
...
var TransferWidget = ({
  ...
  const amount = useSelector42(selectAmount);
  const feeDeduct = useSelector42(selectFeeDeduct);
  const sourceChain = useSelector42(selectSourceChain);
  const sourceAddress = useSelector42(selectSourceAddress);
  const targetAddress = useSelector42(selectTargetAddress);
  const targetChain = useSelector42(selectTargetChain);
  const transactionOption = useSelector42(selectTransactionOption);
  const compliantOption = useSelector42(selectCompliantOption);
  const errorHandler = useSelector42(selectErrorHandler);
  const keplrHandler = useSelector42(selectKeplrHandler);
  const closeHandler = useSelector42(selectCloseHandler);
  const sourceCurrency = useSelector42(selectSourceCurrency);
  const targetCurrency = useSelector42(selectTargetCurrency);
  const backendUrl = useSelector42(selectBackendUrl);
  const networkOption = useSelector42(selectNetworkOption);
  const { totalFeeUsd, targetNetworkFee } = useSelector42(selectServiceFee);
  ...
  const pendingTxs = useSelector42(selectPendingTxs);
  ...
  useEffect24(() => {
    if (sourceComplianceError || targetComplianceError)
      toast10.error("Compliance check failed", {
        icon: /* @__PURE__ */ React170.createElement(Error_default, null)
      });
  }, [sourceComplianceError, targetComplianceError]);
  const handleSubmit = async () => {
    if (totalFeeUsd < 0) {
      toast10.error("Fee is not calculated!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
      errorHandler("Fee is not calculated!");
      return;
    }
    if (dAppOption !== "LPDrain" /* LPDrain */ && balance < (feeDeduct ? +amount : +amount + totalFeeUsd)) {
      toast10.error("Insufficient balance!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
    ...
    try {
      ...
      if (result?.code !== 0) {
        errorHandler(result);
        toast10.error("Failed to submit transaction!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
        setSubmitting(false);
        return;
      }
      let txId = -1;
      ...
      toast10.error("rpc disconnected", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
      toast10.error("Failed to submit transaction", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
    }
  };
  const onNext = () => {
    if (isWizard && wizardStep < 5) {
      if (wizardStep === 1 && !sourceAddress) {
        toast10.error("Wallet is not connected!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
      ...
      if (totalFeeUsd > 0 && totalFeeUsd > +amount && feeDeduct) {
        toast10.error("Fee is greater than amount to transfer!", {
          icon: /* @__PURE__ */ React170.createElement(Error_default, null)
        });
    ...
    if (!isWizard && !formStep) {
      if (sourceAddress) {
        if (mode === "payment" /* payment */ && !transactionOption) {
          toast10.error("Invalid payment details!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
          errorHandler("Invalid payment details!");
          return;
        }
        if (mode === "bridge" /* bridge */ && +amount <= 0) {
          toast10.error("Invalid amount!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
        ...
        if (!targetAddress) {
          toast10.error("Invalid target address!", { icon: /* @__PURE__ */ React170.createElement(Error_default, null) });
