...
function useIsWalletReady(): {
  ...
  const externalProvider = useSelector(selectExternalProvider) as ExternalProvider | undefined;
  ...
  const switchNetwork = useCallback(async () => {
    ...
    if (evmProvider && correctEvmNetwork && appKitModel !== null) {
      try {
        ...
        console.debug('useIsWalletReady:EVM:Network switch successful to:', correctEvmNetwork.name);
      } catch (e) {
  ...
  useEffect(() => {
    ...
    if (externalProvider?.type === 'evm') {
      ...
      if (externalProviderChainId !== expectedChainId) {
        console.warn('useIsWalletReady:EVM:External wallet connected but chain mismatch:', {
    ...
    if (!externalProvider && !isConnected) {
      ...
      console.warn('useIsWalletReady:EVM:Wallet connected but chain mismatch:', {
  ...
  const returnValue = useMemo(() => {
    const ready = externalProvider?.type === 'evm'
    ...
    console.debug('useIsWalletReady:EVM:Final return values:', {
      isReady: ready,
      statusMessage: msg,
      walletAddress: externalProvider?.type === 'evm'
