...
const NetworkSelector: React.FC<NetworkSelectorProps> = ({ type }) => {
  ...
  const networks = useMemo(() => {
    if (isSourceSelector) {
      return networkOptions; // Source selector includes all networks
    }
    return networkOptions.filter((network) => network.id !== sourceNetwork); // Exclude source from target options
  }, [networkOptions, sourceNetwork, isSourceSelector]);
